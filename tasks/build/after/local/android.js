(function() {
  ...
  module.exports = android = function(grunt) {
    ...
    tasks = {
      ...
      setAndroidDebuggable: require('./android/debuggable')(grunt).set
    };
    return {
      run: function(fn) {
        return fluid(tasks).repairVersionCode().setMinSdkVersion().setTargetSdkVersion().setPermissions().setAndroidApplicationName().setAndroidDebuggable().buildIcons().buildScreens().go(function(err, result) {
          if (err) {
            grunt.fatal(err);
          }
          if (fn) {
